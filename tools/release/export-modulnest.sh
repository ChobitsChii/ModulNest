...
sanitize_schema() {
    local source="app/Database/schema.sql"
    local destination="$TARGET/app/Database/schema.sql"
    [[ -f "$source" ]] || return 0
    mkdir -p "$(dirname "$destination")"
    awk '
        BEGIN { skip = 0 }
        /^[[:space:]]*INSERT[[:space:]]+INTO[[:space:]]+`?news_entries`?($|[[:space:]]|\()/ { skip = 1 }
        /^[[:space:]]*INSERT[[:space:]]+INTO[[:space:]]+`?card_sets`?($|[[:space:]]|\()/ { skip = 1 }
        /^[[:space:]]*INSERT[[:space:]]+INTO[[:space:]]+`?cards`?($|[[:space:]]|\()/ { skip = 1 }
        skip == 1 {
            if ($0 ~ /;[[:space:]]*$/) {
                skip = 0
            }
        ...
        { print }
    ' "$source" > "$destination"
    COPIED_ITEMS+=("app/Database/schema.sql (sanitized)")
}
...
copy_public_assets() {
    ...
    mkdir -p "$TARGET/public/assets/favicons" \
        "$TARGET/public/assets/sneak-preview/posters" \
        "$TARGET/public/assets/fantasy-cards/cards"
    touch "$TARGET/public/assets/favicons/.gitkeep" \
        "$TARGET/public/assets/sneak-preview/posters/.gitkeep" \
        "$TARGET/public/assets/fantasy-cards/.gitkeep" \
        "$TARGET/public/assets/fantasy-cards/cards/.gitkeep"
}

copy_storage_placeholders() {
    mkdir -p "$TARGET/storage/logs" \
        "$TARGET/storage/tools/speech/uploads" \
    ...
    php -r '
        ...
        foreach ($core as $name) {
            $modules[] = [
                ...
                "access_level" => $name === "Admin" || $name === "Modules" ? "admin" : "public",
                "description" => "Core-Modul",
                "required" => true,
...
scan_forbidden_paths() {
    ...
    if find "$TARGET/public/assets/favicons" "$TARGET/public/assets/sneak-preview/posters" "$TARGET/public/assets/fantasy-cards/cards" \
        -type f ! -name '.gitkeep' | grep -q .; then
        printf 'Runtime-Assets enthalten Dateien außer .gitkeep.\n' >&2
        find "$TARGET/public/assets/favicons" "$TARGET/public/assets/sneak-preview/posters" "$TARGET/public/assets/fantasy-cards/cards" \
...
scan_schema_safety() {
    ...
    [[ -f "$schema" ]] || return 0
    if grep -nE 'INSERT[[:space:]]+INTO[[:space:]]+(news_entries|card_sets|cards|users|mail_accounts|banking_transactions|sneak_preview_entries)' "$schema" >&2; then
        return 1
    fi
    return 0
}
...
write_summary() {
    ...
    mkdir -p "$(dirname "$summary")"
    {
        ...
        printf -- '- `app/Database/schema.sql` wurde für den Export sanitisiert: keine News-/Karten-Demo-Seeds.\n\n'
...
print_final_summary() {
    ...
    printf '  - app/Core, app/Config, app/Database/schema.sql (sanitized)\n'
