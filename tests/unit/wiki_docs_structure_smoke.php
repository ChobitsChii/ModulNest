...
try {
    ...
    foreach(['development/example-module','modules/wiki','releases/0.5.0','releases/0.9.0','releases/1.0.0','releases/1.0.1','releases/1.1.0','releases/1.1.1','releases/1.2.0','releases/1.3.0','technical/tech-architecture'] as $route) wiki_docs_assert($repo->page($route)!==null,"Expected synchronised documentation route missing: {$route}");
    $navigation=(new WikiNavigationBuilder())->build($repo->pages(),'releases/1.3.0','index');wiki_docs_assert(array_column($navigation['groups'],'label')===['Development','Modules','Releases','Technical'],'The docs tree must produce distinct Development, Modules, Releases and Technical Wiki groups.');$releaseGroup=array_values(array_filter($navigation['groups'],static fn(array $group): bool => $group['label']==='Releases'))[0]??[];wiki_docs_assert(($releaseGroup['is_active']??false)===true&&($releaseGroup['landing']['route_path']??'')==='releases'&&in_array('ModulNest 1.3.0',array_column($releaseGroup['pages']??[],'title'),true),'Release landing pages must not be duplicated as children and releases must remain active.');
