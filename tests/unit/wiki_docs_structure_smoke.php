...
try {
    ...
    foreach(['development/example-module','releases/0.5.0','releases/0.9.0','releases/1.0.0','releases/1.0.1','releases/1.1.0','technical/tech-architecture'] as $route) wiki_docs_assert($repo->page($route)!==null,"Expected synchronised documentation route missing: {$route}");
    $navigation=(new WikiNavigationBuilder())->build($repo->pages(),'releases/1.1.0','index');wiki_docs_assert(array_column($navigation['groups'],'label')===['Development','Releases','Technical'],'The docs tree must produce distinct Development, Releases and Technical Wiki groups.');$releaseGroup=array_values(array_filter($navigation['groups'],static fn(array $group): bool => $group['label']==='Releases'))[0]??[];wiki_docs_assert(($releaseGroup['is_active']??false)===true&&in_array('ModulNest 1.1.0',array_column($releaseGroup['pages']??[],'title'),true),'Release pages must remain grouped, toggleable and active in Wiki navigation.');
