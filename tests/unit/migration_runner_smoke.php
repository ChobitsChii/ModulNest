...
$publicModules = ['Admin', 'Auth', 'Modules', 'User', 'Banking', 'Dashboard', 'DataPortability', 'Logs', 'News', 'SneakPreview', 'Systeminfo', 'Tools', 'Updates'];
...
try {
    ...
    migration_smoke_assert(count($first['executed']) === 5, 'Erster Migrationslauf sollte 5 Migrationen ausführen.');
    ...
    migration_smoke_assert(count($second['skipped']) === 5, 'Zweiter Migrationslauf sollte 5 Migrationen überspringen.');

    $tables = migration_smoke_tables($server);
    foreach (['schema_migrations', 'users', 'modules', 'news_entries', 'dashboard_widgets', 'dashboard_tasks', 'dashboard_notes', 'banking_accounts', 'banking_transactions', 'banking_recurring_rules', 'sneak_preview_entries', 'sneak_preview_settings'] as $table) {
    ...
    migration_smoke_assert($executedCount === 5, 'schema_migrations sollte 5 Einträge enthalten.');
} finally {
...
try {
    ...
    migration_smoke_assert((int) $server->query('SELECT COUNT(*) FROM schema_migrations')->fetchColumn() === 5, 'Migrationen wurden in alter DB nicht markiert.');
} finally {
