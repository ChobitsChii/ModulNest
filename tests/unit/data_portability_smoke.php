...
if (!in_array('sqlite', PDO::getAvailableDrivers(), true)) {
    ...
    $dashboardPdo->exec("INSERT INTO dashboard_tasks (widget_id, title, details, link_url, priority, due_at, is_active, is_done, done_at, archived_at, repeat_type, repeat_time, repeat_weekday, repeat_month_mode, repeat_month_day, repeat_month_ordinal, repeat_month_weekday, sort_order, created_at, updated_at) VALUES (1, 'Archivierte Aufgabe', '', NULL, 'normal', NULL, 1, 0, NULL, '2026-05-02 12:00:00', 'none', NULL, NULL, NULL, NULL, NULL, NULL, 1, '2026-05-01 00:00:00', '2026-05-01 00:00:00')");
    $dashboardPdo->exec("INSERT INTO dashboard_notes (widget_id, title, content, textarea_height, sort_order, is_pinned, is_archived, archived_at, created_at, updated_at) VALUES (1, 'Archivierte Notiz', 'Archivinhalt', NULL, 1, 0, 1, '2026-05-03 12:00:00', '2026-05-01 00:00:00', '2026-05-01 00:00:00')");
    ...
    assert_true((int) $dashboardPdo->query("SELECT COUNT(*) FROM dashboard_tasks WHERE archived_at IS NOT NULL AND widget_id IN (SELECT id FROM dashboard_widgets WHERE user_id = 2)")->fetchColumn() === 1, 'Dashboard-Import stellt archivierte Aufgaben nicht wieder her.');
    assert_true((int) $dashboardPdo->query("SELECT COUNT(*) FROM dashboard_notes WHERE archived_at IS NOT NULL AND widget_id IN (SELECT id FROM dashboard_widgets WHERE user_id = 2)")->fetchColumn() === 1, 'Dashboard-Import stellt archivierte Notizen nicht wieder her.');
