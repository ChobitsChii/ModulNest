...
update_channel_assert(str_contains($html, 'id="collapseUpdateChannel"'), 'Updatekanal-Konfiguration ist nicht standardmäßig eingeklappt.');
update_channel_assert(str_contains($html, 'Vorabversionen aktiviert') && str_contains($html, 'Kanal einstellen'), 'Kompakter Kanalstatus fehlt.');
update_channel_assert(str_contains($html, 'aria-expanded="false"') && str_contains($html, 'history-chevron'), 'Zugänglicher Disclosure-Zustand oder Chevron fehlt.');
