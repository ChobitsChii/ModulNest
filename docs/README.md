# ModulNest Dokumentation

Diese Dokumentation begleitet die Installation, den Betrieb und die Weiterentwicklung von ModulNest.

## Installation und Betrieb

- [Installation](installation.md) und [Anforderungen](requirements.md)
- [Konfiguration](configuration.md) und [Datenbank](database.md)
- [Module](modules/README.md) und [Routen](routes.md)
- [Tests](testing.md) sowie [Technologien und Abhängigkeiten](dependencies.md)

## Entwicklung

Die [Developer-Dokumentation](development/README.md) beschreibt den nativen Modulvertrag, Sicherheit, Tests und den Lebenszyklus von Modulen.

- [Modul erstellen](development/create-module.md)
- [Modul-Generator](development/create-module.md#generator-optional)
- [Modulspezifikation](development/module-spec.md)
- [Vollständiges Example-Modul](development/example-module.md)
- [Sicherheitsregeln](development/security.md)
- [Tests für Module](development/testing.md)

## Betrieb und Releases

- [Update- und Release-Prozess](release.md)
- [Release-Historie](releases/README.md)
- [Module](modules/README.md)
- [Export und Import](export-import.md)
- [Technische Dokumentation](technical/README.md)
- [Dashboard-Grundlage](technical/dashboard-foundation.md)
